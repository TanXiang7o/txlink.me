...
转载自https://github.com/apache/rocketmq/blob/develop/docs/cn/best_practice.md
...
一个应用尽可能用一个Topic，而消息子类型则可以用tags来标识。tags可以由应用自由设置，只有生产者在发送消息设置了tags，消费方在订阅消息时才可以利用tags通过broker做**消息过滤**：message.setTags("TagA")。  

#### 1.1.2 Keys的使用

每个消息在业务层面的唯一标识码要设置到keys字段，方便将来**定位消息丢失问题**。服务器会为**每个消息创建索引（哈希索引）**，应用可以通过topic、key来查询这条消息内容，以及消息被谁消费。由于是哈希索引，请务必保证key尽可能唯一，这样可以避免潜在的哈希冲突。
...
Producer的**send方法本身支持内部重试**，重试逻辑如下：
...
msgId一定是全局唯一标识符，但是实际使用中，可能会存在**相同的消息有两个不同msgId的情况**（消费者主动重发、因客户端重投机制导致的重复等），这种情况就需要使业务字段进行重复消费。
...
客户端启动后，会定时访问一个静态HTTP服务器，地址如下：http://jmenv.tbsite.net:8080/rocketmq/nsaddr
这个URL的返回内容如下：
...
3. **RocketMQ生产者发送消息时，如何处理发送失败的情况？**

   Producer的send方法本身支持内部重试，至多重试2次。如果同步模式发送失败，则轮转到下一个Broker，如果异步模式发送失败，则只会在当前Broker进行重试。如果业务对消息可靠性要求比较高，建议应用增加相应的重试逻辑，比如调用send同步方法发送失败时，则尝试将消息存储到db，然后由后台线程定时重试，确保消息一定到达Broker。
4. **如果消息消费速度慢，我该怎么办？**
   增加同一消费者组下的消费者数量（机器或者进程）、增加并发消费的线程数量、批量处理消息、跳过非重要消息，并优化消费过程中的I/O交互。
5. **RocketMQ如何处理消息重复？**
   RocketMQ并不防止重复；在您的应用程序中实施幂等处理至关重要。虽然MsgId可以全局唯一，但是可能会有同一条消息被多次发送。
6. **RocketMQ中的FlushDiskType有哪些，它们有什么区别？**

   FlushDiskType有SYNC_FLUSH（同步刷新）和ASYNC_FLUSH（异步处理）两种。SYNC_FLUSH相比于ASYNC_FLUSH会损失很多性能，但是也更可靠，所以需要根据实际的业务场景做好权衡。
7. **推荐哪些JVM设置用于RocketMQ？**
